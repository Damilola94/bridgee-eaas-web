...
function SendInvite({ onClose }: Props) {
  ...
  const sendInviteMutation = useMutation(handleFetch, {
    onSuccess: (res: any) => {
      notification({
        message:
          res?.message || `You have successfully sent an invite to ${email}`,
        type: "success",
      });
      onClose();
    },
    ...
    <Modal isOpen onClose={onClose} isCenter maxWidth="max-w-[400px]">
      ...
      <div className="w-full pb-3">
        ...
        <div className="w-full my-3 text-center relative">
          <hr />
          <p className="font-bold bg-white inline-block px-5 relative -top-2.5">
            or
          </p>
        </div>
        <div className="w-full">
          <Button
            ...
            paddingY="p-3"
            bgColor="bg-primary"
            onClick={() => copy("https://bridgebyalat.ng?r=30303333")}
          >
            {isCopied ? (
